...
const LS_KEY = 'gap-stats:state'
export default function GapStatsPage() {
 ...
 const run = async (tk: string, w: string) => {
  const t = (tk || ticker).toUpperCase().trim()
  if (!t) return
 ...
 // restore last session on mount + auto-run it
 useEffect(() => {
  try {
  ...
  <div className="min-h-screen bg-[#0a0a0a] text-[#e0e0e0]">
   ...
   <div className="border-b border-[#1a1a1a] px-6 py-4">
    <div className="max-w-[1400px] mx-auto flex items-center justify-between">
     <div>
      <h1 className="text-xl font-bold text-[#D4AF37]">Gap Stats</h1>
      <p className="text-xs text-[#666] mt-0.5">Intraday microstructure of gap days — fade rates, time-of-high/low, post-trigger HOD reclaim</p>
     </div>
     {ticker && !loading && (
      <div className="text-right">
       <div className="text-2xl font-bold text-[#e0e0e0]">{ticker}</div>
       <div className="text-xs text-[#666]">{win} · {data?.asOf?.slice(0, 10)}</div>
      </div>
     )}
    </div>
   </div>
   ...
   <div className="px-6 py-4 border-b border-[#1a1a1a]">
    <div className="max-w-[1400px] mx-auto flex items-center gap-3">
     <form onSubmit={e => { e.preventDefault(); run(input, win) }} className="flex items-center gap-2">
      <input
       value={input}
       ...
       autoFocus
      />
      <button type="submit" className="bg-[#D4AF37] text-[#0a0a0a] font-bold rounded-lg px-5 py-2 text-sm hover:opacity-90 transition-opacity">
       Analyze
      </button>
     </form>
   ...
   <div className="max-w-[1400px] mx-auto px-6 py-6 flex gap-6 items-start">
    ...
    <aside className="w-44 shrink-0">
     ...
     <div className="flex flex-col gap-0.5">
      {history.map(t => (
       <button key={t} onClick={() => { setInput(t); run(t, win) }}
        className={`text-left px-2.5 py-1.5 rounded-lg text-sm transition-colors ${t === ticker ? 'bg-[#D4AF37]/15 text-[#D4AF37] border border-[#D4AF37]/30' : 'text-[#9ca3af] hover:bg-[#141c2b] hover:text-[#e0e0e0] border border-transparent'}`}>
        {t}
       </button>
      ))}
      {history.length === 0 && <div className="text-xs text-[#444] px-2.5 py-1">No history yet</div>}
     </div>
    </aside>
