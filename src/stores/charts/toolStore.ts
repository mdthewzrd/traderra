...
export const IND_CATALOG: Record<string, IndCatalogEntry> = {
  ...
  {key:'structSuppress',label:'Counter-Trend Suppress',def:1,type:'toggle',group:'aop'},
      ...
      {key:'tlLeft',label:'TL Look Left',def:50,min:1,max:200,group:'trendline'},
      {key:'tlRight',label:'TL Look Right',def:15,min:1,max:120,group:'trendline'},
      {key:'tlPattern',label:'TL Pattern',def:2,min:1,max:13,group:'trendline'},
      {key:'tlMinSize',label:'TL Min Size (×ATR)',def:0,step:0.1,min:0,max:5,group:'trendline'},
