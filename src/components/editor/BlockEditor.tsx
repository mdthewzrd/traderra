...
import { EnhancedBlockSelector } from './EnhancedBlockSelector'
...
export interface BlockEditorProps {
  ...
  mode?: 'rich-text' | 'blocks' // New mode selector
}
...
export function BlockEditor({
  ...
  onChange,
  onSave,
  placeholder = "Start writing... Type '/' for commands or '+' for blocks",
  editable = true,
  showToolbar = true,
    ...
    editorProps: {
      attributes: {
        class: cn(
          ...
          '[&_.trade-entry-block]:my-4',
          '[&_.strategy-template-block]:my-4',
          '[&_.performance-summary-block]:my-4',
          className
        ),
      },
      handleKeyDown: (view, event) => {
        ...
        // Handle plus key for block selector (alternative trigger)
        if (event.key === '+' && !event.shiftKey && !event.ctrlKey && !event.metaKey) {
          const { selection } = view.state
          const { $from } = selection
        ...
        // Block navigation with arrow keys when block is selected
      ...
      handleClick: (view, pos, event) => {
        ...
        if (node && (node.type.name === 'tradeEntry' || node.type.name === 'strategyTemplate' || node.type.name === 'performanceSummary')) {
          setSelectedBlockId(node.attrs.id)
          return true
        } else {
  ...
  // Handle block selection
  const handleBlockSelect = useCallback((blockType: string, data?: any) => {
    if (!editor) return

    const { selection } = editor.state
    const { from } = selection

    // Remove the trigger character if it was typed
    const triggerChar = editor.state.doc.textBetween(from - 1, from)
    ...
    switch (blockType) {
      ...
      case 'horizontalRule':
        ...
        break
      case 'tradeEntry':
        editor.chain().focus().insertTradeEntry(data).run()
        break
      case 'strategyTemplate':
        editor.chain().focus().insertStrategyTemplate(data).run()
        break
      case 'performanceSummary':
        editor.chain().focus().insertPerformanceSummary(data).run()
        break
      case 'image':
        // Handle image upload
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-sm studio-muted">Loading block editor...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full studio-bg">
      ...
      <div className="flex-1 relative" ref={editorRef}>
        ...
        <DragDropHandler editor={editor} />

        <EditorContent
          editor={editor}
          className="h-full studio-surface rounded-lg border border-studio-border overflow-auto relative"
        />

        {/* Enhanced Block Selector */}
        {isBlockSelectorOpen && (
          <EnhancedBlockSelector
      ...
      {/* Enhanced Character Count with Block Info */}
        ...
        <div className="flex items-center justify-between p-2 text-xs studio-muted border-t border-studio-border">
          <div className="flex items-center gap-4">
            ...
            {mode === 'blocks' && (
              <>
                <span>•</span>
                <span>{countBlocks(editor.getJSON())} blocks</span>
              </>
            )}
          </div>
...
function countBlocks(content: JSONContent): number {
  ...
  function traverse(node: JSONContent) {
    if (node.type === 'tradeEntry' || node.type === 'strategyTemplate' || node.type === 'performanceSummary') {
      count++
    }
    if (node.content) {
