...
// Generate sample trades for demo purposes (same as calendar page)
const generateSampleTrades = (year: number) => {
  ...
  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
    ...
    if (Math.random() > 0.3) {
      ...
      // Generate random entry hour (market hours: 9:30 AM - 4:00 PM)
      const entryHour = Math.floor(Math.random() * 8) + 9 // 9-16 (9 AM to 4 PM)
      const entryMinute = Math.random() > 0.5 ? '00' : '30'
      const entryTime = `${entryHour}:${entryMinute} AM`

      // Generate random duration (30m to 6h 30m)
      const durationHours = Math.floor(Math.random() * 6) + 0.5 // 0.5 to 6.5 hours
      const durationMinutes = Math.round(durationHours * 60)
      const duration = `${durationHours >= 1 ? `${Math.floor(durationHours)}h ` : ''}${durationMinutes % 60}m`.trim()
...
function StatisticsPageContent() {
  ...
  // Register component handlers with AG-UI registry
  ...
  useComponentRegistry('statistics.tabs.overview', {
    activate: (action) => {
      if (action === 'activate' || action === 'click') {
  ...
  useComponentRegistry('statistics.tabs.performance', {
    activate: (action) => {
      if (action === 'activate' || action === 'click') {
  ...
  useComponentRegistry('statistics.charts', {
    scroll: (behavior) => {
  ...
  useComponentRegistry('statistics.metrics', {
    scroll: (behavior) => {
  ...
  // Listen for AG UI stats tab changes (legacy support - will be deprecated)
  useEffect(() => {
    const handleStatsTabChange = (event: Event) => {
      const customEvent = event as CustomEvent<{ tab: 'overview' | 'analytics' | 'performance' }>
      console.log('[AG-UI] Stats tab change event:', customEvent.detail)
      setActiveTab(customEvent.detail.tab)
    }

    window.addEventListener('statsTabChange', handleStatsTabChange)

    return () => {
      window.removeEventListener('statsTabChange', handleStatsTabChange)
    }
  }, [])
  ...
  useComponentRegistry('statistics.filters.symbol', {
    setState: (state) => {
      if (typeof state === 'string') {
        setFilters(prev => ({ ...prev, symbol: state }))
      }
    }
  })

  useComponentRegistry('statistics.filters.tags', {
    setState: (state) => {
      if (typeof state === 'string') {
        setFilters(prev => ({ ...prev, tags: state }))
      }
    }
  })
  ...
  useComponentRegistry('statistics.filters.side', {
    setState: (state) => {
      if (typeof state === 'string' && ['All', 'Long', 'Short'].includes(state)) {
        setFilters(prev => ({ ...prev, side: state }))
      }
    }
  })

  useComponentRegistry('statistics.filters.duration', {
    setState: (state) => {
      if (typeof state === 'string') {
        setFilters(prev => ({ ...prev, duration: state }))
      }
    }
  })

  useComponentRegistry('statistics.filters.show', {
    activate: (action) => {
      if (action === 'open' || action === 'show' || action === 'toggle') {
        setShowFilters(true)
  ...
  useComponentRegistry('statistics.filters.clear', {
    activate: (action) => {
      if (action === 'clear' || action === 'reset') {
        setFilters({ symbol: '', tags: '', side: 'All', duration: 'All' })
      }
    }
  })
  ...
  // Use sample trades if no real trades exist (for demo purposes)
  const currentYear = new Date().getFullYear()
  const sampleTrades = useMemo(() => (!trades || trades.length === 0) ? generateSampleTrades(currentYear) : [], [currentYear, trades])
  const displayTrades = (trades && trades.length > 0) ? trades : sampleTrades

  // Apply date filtering to trades
  const filteredTrades = getFilteredData(displayTrades || [])

  // Apply additional filters (symbol, tags, side, duration)
  const actuallyFilteredTrades = useMemo(() => {
    ...
    // Apply tags/strategy filter
    if (filters.tags) {
      result = result.filter(trade => {
        const strategy = trade.strategy || ''
        return strategy.toLowerCase().includes(filters.tags.toLowerCase())
      })
    }
    ...
    if (filters.duration !== 'All') {
      result = result.filter(trade => {
        if (!trade.duration) return false
        const tradeDuration = trade.duration.toString().toLowerCase()
        const filterDuration = filters.duration.toLowerCase()
        return tradeDuration.includes(filterDuration) || filterDuration.includes(tradeDuration)
      })
    }
  ...
  const tagFilteredTrades = getTagFilteredTrades(actuallyFilteredTrades, selectedTag)

  // Calculate statistics from filtered trade data
  const stats = calculateTradeStatistics(actuallyFilteredTrades, mode)

  // Expose statistics page data to Renata AI for context awareness
  useCopilotReadable({
    description: 'Statistics page with comprehensive trading performance metrics and analysis',
    value: {
      ...
      pnlMode: mode,
      filteredTradeCount: actuallyFilteredTrades.length,
      totalTradeCountBeforeFilters: filteredTrades.length,
      activeFilters: {
        symbol: filters.symbol || null,
        tags: filters.tags || null,
        side: filters.side !== 'All' ? filters.side : null,
        duration: filters.duration !== 'All' ? filters.duration : null
      },
      hasActiveFilters: !!(filters.symbol || filters.tags || filters.side !== 'All' || filters.duration !== 'All')
    }
  })
  ...
  // Calculate win rate by hour data - Performance optimized
  const winRateByHourData = useMemo(() => {
    // Calculate win rate by hour of day (market hours: 4 AM - 4 PM)
    const hourStats: { [key: string]: { totalTrades: number; winners: number; totalR: number } } = {}

    // Initialize hours from 4 AM to 4 PM
    for (let i = 4; i <= 16; i++) {
      const period = i < 12 ? 'AM' : 'PM'
      const hour = i <= 12 ? i : i - 12
      hourStats[`${hour}:00 ${period}`] = { totalTrades: 0, winners: 0, totalR: 0 }
    }

    filteredTrades.forEach(trade => {
      // Parse entry time to get hour
      let hour = '9:30 AM' // Default if no entry time

      if (trade.entryTime) {
        // Extract hour from entryTime (e.g., "9:30 AM" or "09:30:00")
        const timeMatch = trade.entryTime.match(/(\d{1,2}):(\d{2})/)
        if (timeMatch) {
          let h = parseInt(timeMatch[1])
          const period = trade.entryTime.toLowerCase().includes('pm') && h !== 12 ? 'PM' : 'AM'
          if (period === 'PM' && h !== 12) h += 12
          if (period === 'AM' && h === 12) h = 0

          // Convert to display format (e.g., "9:00 AM")
          const displayHour = h > 12 ? h - 12 : (h === 0 ? 12 : h)
          hour = `${displayHour}:00 ${period}`
        }
      } else if (trade.date.includes('T')) {
        // Parse from full datetime
        const dateTime = new Date(trade.date)
    ...
    return Object.keys(hourStats)
      .filter(hour => hourStats[hour].totalTrades > 0) // Only show hours with trades
      .map(hour => {
        const stats = hourStats[hour]
        const winRate = Math.round((stats.winners / stats.totalTrades) * 100)
      ...
      .sort((a, b) => {
        ...
        const aPeriod = a.hour.includes('AM') ? 0 : 12
        const bPeriod = b.hour.includes('AM') ? 0 : 12
        return (aHour + aPeriod) - (bHour + bPeriod)
      })
  }, [filteredTrades, mode])

  // Calculate average winner size by hour data - Performance optimized
  const averageWinnerByHourData = useMemo(() => {
    // Calculate average winner size by hour of day (market hours: 4 AM - 4 PM)
    const hourStats: { [key: string]: { winners: number; totalWinnerR: number; totalR: number } } = {}

    // Initialize hours from 4 AM to 4 PM
    for (let i = 4; i <= 16; i++) {
      const period = i < 12 ? 'AM' : 'PM'
      const hour = i <= 12 ? i : i - 12
      hourStats[`${hour}:00 ${period}`] = { winners: 0, totalWinnerR: 0, totalR: 0 }
    }
    ...
    // Calculate max average winner for scaling
    const avgWinners = Object.keys(hourStats)
      .filter(hour => hourStats[hour].winners > 0)
      .map(hour => hourStats[hour].totalWinnerR / hourStats[hour].winners)
    const maxAvgWinner = Math.max(...avgWinners, 0.1) // Prevent division by zero

    return Object.keys(hourStats)
      .filter(hour => hourStats[hour].winners > 0) // Only show hours with winners
      .map(hour => {
        const stats = hourStats[hour]
        const avgWinner = stats.winners > 0 ? stats.totalWinnerR / stats.winners : 0
        const avgWinnerDollars = avgWinner * 100 // Convert R to dollars (assuming 1R = $100)
        ...
        return {
          ...
          avgWinner,
          avgWinnerDollars,
          percentage: maxAvgWinner > 0 ? (avgWinner / maxAvgWinner) * 100 : 0,
          colors,
          totalR: stats.totalR,
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="studio-surface rounded-lg p-6 bg-[#1a1a1a] border border-gray-700/50">
                  ...
                  <div className="h-64 overflow-y-auto pr-4">
                    ...
                    <div className="space-y-1 text-sm">
                      {winRateByHourData.length > 0 ? (
                        winRateByHourData.map(({ hour, winRate, colors, totalR }) => (
                          <div key={hour} className="flex items-center py-1 space-x-3">
                            <div className="w-16 text-xs studio-text text-right">{hour}</div>
                        ...
                        <div className="text-center py-8 text-sm studio-muted">
                          No hourly data available - trades need entry times
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="studio-surface rounded-lg p-6 bg-[#1a1a1a] border border-gray-700/50">
                  ...
                  <div className="h-64 overflow-y-auto pr-4">
                    ...
                    <div className="space-y-1 text-sm">
                      {averageWinnerByHourData.length > 0 ? (
                        averageWinnerByHourData.map(({ hour, avgWinner, avgWinnerDollars, percentage, colors, totalWinners }) => (
                          <div key={hour} className="flex items-center py-1 space-x-3">
                            <div className="w-16 text-xs studio-text text-right">{hour}</div>
                            <div className="flex-1 relative h-4 bg-[#1a1a1a] rounded-xs overflow-hidden">
                              <div
                            ...
                            <div className="text-right w-24">
                              <span className={`font-medium text-xs ${colors.text}`}>
                                {displayMode === 'r' ? `${avgWinner.toFixed(1)}R` : `$${Math.round(avgWinnerDollars)}`}
                              </span>
                              <span className="text-xs studio-muted ml-1">({totalWinners})</span>
                            </div>
                          </div>
                        ))
                      ) : (
                ...
                {(!trades || trades.length === 0) && (
                  <div className="mb-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-yellow-500 font-medium">Demo Mode - Showing sample data</span>
                    </div>
                    <span className="text-xs text-yellow-500/70">Import your trades to see real statistics</span>
                  </div>
                )}
                {renderTabContent()}
              </>
