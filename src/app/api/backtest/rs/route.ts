...
export async function GET() {
  try {
    ...
    try {
      ...
      return NextResponse.json({ marks: [], grades: {} })
    } finally {
      await prisma.$disconnect()
    }
  } catch (e: any) {
    return NextResponse.json({ error: e.message, marks: [], grades: {} }, { status: 500 })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      await prisma.$executeRawUnsafe(`INSERT INTO "_backtest_kv" (key, value, "updatedAt") VALUES ('rs-marks', '${escaped}', NOW()) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, "updatedAt" = NOW()`)
