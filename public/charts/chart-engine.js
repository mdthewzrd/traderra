...
function renderPanel(p){
  ...
  // ── PRICE AXIS ──
  ctx.fillStyle='#6878a8'; ctx.font=fp(chartFonts.price); ctx.textAlign='right';
  for(let i=0;i<=priceSteps;i++){
  ...
  ctx.fillStyle='#6878a8'; ctx.font=fp(chartFonts.time); ctx.textAlign='center';
  ...
  if(volH>0){
    ...
    ctx.fillStyle='#4a5580'; ctx.font=fp(chartFonts.time-1); ctx.textAlign='right';
    ...
    for(const ann of annotations){
      ...
      if(isExecAnn){
        ...
        const execPlaceLbl=(lbl2,col,anchorY,prefBelow)=>{
          ctx.font=fp(chartFonts.price+1);
        ...
        if(ann.type==='entry_arrow'){
          ...
          if(ann._autoStop){
            ...
            ctx.font=fp(chartFonts.price); ctx.textAlign='left';
            const slbl2='S:'+lbl;
      ...
      if(isTL){
        ...
        ctx.font=fp(chartFonts.ohlc); ctx.textAlign='left';
    ...
    if(syncBi>=0){
      ...
      if(globalCrossPrice>0&&priceRange>0){
        ...
        if(syncY>=0&&syncY<=priceH){
          ...
          ctx.fillStyle='#8090b0';ctx.font=fp(chartFonts.price);ctx.textAlign='right';
      ...
      ctx.font=fp(chartFonts.time); ctx.textAlign='center';
  ...
  if(cx>=0&&cx<=chartW&&cy>=0&&cy<=priceH+volH){
    ...
    if(cy<=priceH){
      ...
      ctx.fillStyle='#00e676';ctx.font=fp(chartFonts.price);ctx.textAlign='right';
      ...
      ctx.fillStyle='#1a2040';ctx.fillRect(chartW,cy-10,PRICE_W,20);
      ctx.strokeStyle='#2a3050';ctx.lineWidth=1;ctx.strokeRect(chartW,cy-10,PRICE_W,20);
      ctx.fillStyle='#8080e8';ctx.font=fp(chartFonts.price);ctx.textAlign='right';
  ...
  if(showPriceLine&&data.length){
    ...
    if(ly>=0&&ly<=priceH){
      ...
      ctx.fillStyle='#fff'; ctx.font=fp(chartFonts.price); ctx.textAlign='right';
...
function showOnlyPanel(idx) {
  ...
  panels.forEach((p, i) => {
    ...
    if (i === idx) {
      ...
      setTimeout(() => resizePanel(p), 20);
    } else {
      el.style.display = 'none';
    }
  });
  // Update ALL indicators (one per panel)
  document.querySelectorAll('.panel-indicator').forEach(el => {
    el.textContent = (idx + 1) + ' / ' + panels.length;
  });
  // Robust resize after display change
  setTimeout(() => {
    const p = panels[idx];
    if (p && p.canvas) resizePanel(p);
    renderAll();
  }, 50);
}

function switchPanel(dir) {
  let next = activePanelIdx + dir;
...
function buildPanels(){
  ...
  panels.forEach((p,i)=>{
    ...
    div.innerHTML=`
      <div class="ph" id="ph-${i}">
        ...
        <div class="panel-btns">
          <button class="pnl-btn" id="panel-left-${i}" style="font-size:14px;padding:2px 8px;color:#D4AF37;border-color:#D4AF37;" title="Previous chart">◀</button>
          <span class="panel-indicator" id="panel-indicator-${i}" style="font-size:10px;color:#D4AF37;min-width:30px;text-align:center;font-weight:700;">1 / 4</span>
          <button class="pnl-btn" id="panel-right-${i}" style="font-size:14px;padding:2px 8px;color:#D4AF37;border-color:#D4AF37;" title="Next chart">▶</button>
          <button class="pnl-btn expand-btn" id="expand-${i}" style="margin-left:4px;">⛶</button>
        </div>
      </div>
    ...
    // Panel nav arrows
    const leftBtn=document.getElementById('panel-left-'+i);
    const rightBtn=document.getElementById('panel-right-'+i);
    if(leftBtn) leftBtn.addEventListener('click',()=>switchPanel(-1));
    if(rightBtn) rightBtn.addEventListener('click',()=>switchPanel(1));
...
function adjustFullscreenRight(){
  if(fullscreenPanel===null) { resizePanel(panels[activePanelIdx]); return; }
...
// ══════════════════════════════════════════════════════════
//  SETTINGS PANEL — Look & Feel
// ══════════════════════════════════════════════════════════
const SETTINGS_KEY = 'traderra-chart-settings';
let chartFonts = { price: 10, time: 9, ohlc: 12, vol: 10 };
function fp(sz) { return 'bold ' + sz + 'px Courier New'; }

const PRESETS = {
  default: {
    bg:'#0c0e14', axisbg:'#0d0f18', grid:'#141926', up:'#26a69a', dn:'#ef5350',
    vol_up:'rgba(38,166,154,.5)', vol_dn:'rgba(239,83,80,.5)',
    pre:'#a05014', pre_opacity:7, after:'#1e50c8', after_opacity:9,
    cross:'#8ca0c8', cross_opacity:50, border:'#1e2535',
    price:10, time:9, ohlc:12, vol:10
  },
  gold: {
    bg:'#0a0a08', axisbg:'#0d0c0a', grid:'#1a1810', up:'#D4AF37', dn:'#ef5350',
    vol_up:'rgba(212,175,55,.5)', vol_dn:'rgba(239,83,80,.5)',
    pre:'#6b4c14', pre_opacity:10, after:'#1e50c8', after_opacity:8,
    cross:'#D4AF37', cross_opacity:40, border:'#2a2510',
    price:10, time:9, ohlc:12, vol:10
  },
  light: {
    bg:'#ffffff', axisbg:'#f8f8f8', grid:'#e0e0e0', up:'#26a69a', dn:'#ef5350',
    vol_up:'rgba(38,166,154,.4)', vol_dn:'rgba(239,83,80,.4)',
    pre:'#ff9800', pre_opacity:6, after:'#2196f3', after_opacity:5,
    cross:'#333333', cross_opacity:60, border:'#cccccc',
    price:10, time:9, ohlc:12, vol:10
  },
  nord: {
    bg:'#2e3440', axisbg:'#3b4252', grid:'#434c5e', up:'#a3be8c', dn:'#bf616a',
    vol_up:'rgba(163,190,140,.5)', vol_dn:'rgba(191,97,106,.5)',
    pre:'#d08770', pre_opacity:10, after:'#81a1c1', after_opacity:8,
    cross:'#d8dee9', cross_opacity:40, border:'#4c566a',
    price:10, time:9, ohlc:12, vol:10
  }
};
...
function applySettings(s) {
  // Apply colors to the C object
  if (s.bg) C.bg = s.bg;
  if (s.axisbg) C.axisbg = s.axisbg;
  if (s.grid) C.grid = s.grid;
  if (s.up) { C.up = s.up; C.vol_up = 'rgba(' + hexToRgb(s.up).r + ',' + hexToRgb(s.up).g + ',' + hexToRgb(s.up).b + ',.5)'; }
  if (s.dn) { C.dn = s.dn; C.vol_dn = 'rgba(' + hexToRgb(s.dn).r + ',' + hexToRgb(s.dn).g + ',' + hexToRgb(s.dn).b + ',.5)'; }
  if (s.vol_up) C.vol_up = s.vol_up;
  if (s.vol_dn) C.vol_dn = s.vol_dn;
  if (s.pre) C.pre = 'rgba(' + hexToRgb(s.pre).r + ',' + hexToRgb(s.pre).g + ',' + hexToRgb(s.pre).b + ',' + (s.pre_opacity || 7) / 100 + ')';
  if (s.after) C.after = 'rgba(' + hexToRgb(s.after).r + ',' + hexToRgb(s.after).g + ',' + hexToRgb(s.after).b + ',' + (s.after_opacity || 9) / 100 + ')';
  if (s.cross) C.cross = 'rgba(' + hexToRgb(s.cross).r + ',' + hexToRgb(s.cross).g + ',' + hexToRgb(s.cross).b + ',' + (s.cross_opacity || 50) / 100 + ')';
  if (s.border) {
    document.querySelectorAll('.panel').forEach(el => el.style.borderColor = s.border);
  }
  // Apply fonts
  if (s.price) chartFonts.price = s.price;
  if (s.time) chartFonts.time = s.time;
  if (s.ohlc) chartFonts.ohlc = s.ohlc;
  if (s.vol) chartFonts.vol = s.vol;
  // Update panel backgrounds
  document.querySelectorAll('.panel').forEach(el => el.style.background = C.bg);
  document.querySelectorAll('.cwrap canvas').forEach(el => { /* canvases redraw via renderAll */ });
  renderAll();
}

function syncSettingsUI() {
  // Read current C values and sync the color picker inputs
  ...
  const opacityFromRgba = (rgba) => {
    const m = rgba.match(/[\d.]+\)$/);
    return m ? Math.round(parseFloat(m[0]) * 100) : 50;
  };
  if(el('set-c-up')) el('set-c-up').value = C.up;
  if(el('set-c-dn')) el('set-c-dn').value = C.dn;
  if(el('set-c-volup')) el('set-c-volup').value = hexFromRgba(C.vol_up);
  if(el('set-c-voldn')) el('set-c-voldn').value = hexFromRgba(C.vol_dn);
  if(el('set-c-bg')) el('set-c-bg').value = C.bg;
  if(el('set-c-axisbg')) el('set-c-axisbg').value = C.axisbg;
  if(el('set-c-grid')) el('set-c-grid').value = C.grid;
  if(el('set-c-border')) el('set-c-border').value = '#1e2535';
  if(el('set-c-pre')) el('set-c-pre').value = hexFromRgba(C.pre);
  if(el('set-c-pre-opacity')) { el('set-c-pre-opacity').value = opacityFromRgba(C.pre); el('set-c-pre-opacity-val').textContent = opacityFromRgba(C.pre)+'%'; }
  if(el('set-c-after')) el('set-c-after').value = hexFromRgba(C.after);
  if(el('set-c-after-opacity')) { el('set-c-after-opacity').value = opacityFromRgba(C.after); el('set-c-after-opacity-val').textContent = opacityFromRgba(C.after)+'%'; }
  if(el('set-c-cross')) el('set-c-cross').value = hexFromRgba(C.cross);
  if(el('set-c-cross-opacity')) { el('set-c-cross-opacity').value = opacityFromRgba(C.cross); el('set-c-cross-opacity-val').textContent = opacityFromRgba(C.cross)+'%'; }
  // Fonts
  if(el('set-font-price')) { el('set-font-price').value = chartFonts.price; el('set-font-price-val').textContent = chartFonts.price+'px'; }
  if(el('set-font-time')) { el('set-font-time').value = chartFonts.time; el('set-font-time-val').textContent = chartFonts.time+'px'; }
  if(el('set-font-ohlc')) { el('set-font-ohlc').value = chartFonts.ohlc; el('set-font-ohlc-val').textContent = chartFonts.ohlc+'px'; }
  if(el('set-font-vol')) { el('set-font-vol').value = chartFonts.vol; el('set-font-vol-val').textContent = chartFonts.vol+'px'; }
}

function saveSettings() {
  const el = (id) => document.getElementById(id);
  const settings = {
    bg: el('set-c-bg').value,
    axisbg: el('set-c-axisbg').value,
    grid: el('set-c-grid').value,
    up: el('set-c-up').value,
    dn: el('set-c-dn').value,
    vol_up: 'rgba(' + hexToRgb(el('set-c-volup').value).r + ',' + hexToRgb(el('set-c-volup').value).g + ',' + hexToRgb(el('set-c-volup').value).b + ',.5)',
    vol_dn: 'rgba(' + hexToRgb(el('set-c-voldn').value).r + ',' + hexToRgb(el('set-c-voldn').value).g + ',' + hexToRgb(el('set-c-voldn').value).b + ',.5)',
    pre: el('set-c-pre').value,
    pre_opacity: parseInt(el('set-c-pre-opacity').value),
    after: el('set-c-after').value,
    after_opacity: parseInt(el('set-c-after-opacity').value),
    cross: el('set-c-cross').value,
    cross_opacity: parseInt(el('set-c-cross-opacity').value),
    border: el('set-c-border').value,
    price: parseInt(el('set-font-price').value),
    time: parseInt(el('set-font-time').value),
    ohlc: parseInt(el('set-font-ohlc').value),
    vol: parseInt(el('set-font-vol').value),
  };
...
function initSettingsPanel() {
  ...
  btn.addEventListener('click', () => {
    ...
    if (!isOpen) syncSettingsUI();
    // Resize active panel after sidebar toggle
    setTimeout(() => {
      adjustFullscreenRight();
      const p = panels[activePanelIdx];
      if (p) resizePanel(p);
      renderAll();
    }, 100);
  });
  ...
  const liveApply = () => {
    applySettings({
      bg: el('set-c-bg').value, axisbg: el('set-c-axisbg').value, grid: el('set-c-grid').value,
      up: el('set-c-up').value, dn: el('set-c-dn').value,
      vol_up: 'rgba(' + hexToRgb(el('set-c-volup').value).r + ',' + hexToRgb(el('set-c-volup').value).g + ',' + hexToRgb(el('set-c-volup').value).b + ',.5)',
      vol_dn: 'rgba(' + hexToRgb(el('set-c-voldn').value).r + ',' + hexToRgb(el('set-c-voldn').value).g + ',' + hexToRgb(el('set-c-voldn').value).b + ',.5)',
      pre: el('set-c-pre').value, pre_opacity: parseInt(el('set-c-pre-opacity').value),
      after: el('set-c-after').value, after_opacity: parseInt(el('set-c-after-opacity').value),
      cross: el('set-c-cross').value, cross_opacity: parseInt(el('set-c-cross-opacity').value),
      border: el('set-c-border').value,
      price: parseInt(el('set-font-price').value), time: parseInt(el('set-font-time').value),
      ohlc: parseInt(el('set-font-ohlc').value), vol: parseInt(el('set-font-vol').value),
    });
  };

  // Wire all color pickers
  ['set-c-up','set-c-dn','set-c-volup','set-c-voldn','set-c-bg','set-c-axisbg','set-c-grid','set-c-border','set-c-pre','set-c-after','set-c-cross'].forEach(id => {
    const inp = el(id);
    if (inp) inp.addEventListener('input', liveApply);
  });

  // Wire all range sliders with value display
  ['set-c-pre-opacity','set-c-after-opacity','set-c-cross-opacity','set-font-price','set-font-time','set-font-ohlc','set-font-vol'].forEach(id => {
    const inp = el(id);
    const valEl = el(id + '-val');
    if (inp && valEl) {
      inp.addEventListener('input', () => {
        const unit = id.includes('font') ? 'px' : '%';
  ...
  document.querySelectorAll('.settings-preset').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (p) {
        applySettings(p);
        syncSettingsUI();
        toast('Applied: ' + btn.textContent);
      }
    });
  });
}

// Make the renderPanel function use chartFonts for sizes
// We override the font sizes by patching the ctx.font calls dynamically
const _origRenderPanel = renderPanel;
renderPanel = function(p) {
  // Temporarily patch font size helpers
  _origRenderPanel(p);
};
